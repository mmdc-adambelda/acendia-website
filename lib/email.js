...
/**
 * Notifies support@acendia.agency of a new lead-magnet (ebook/guide)
 * form submission. Hardcoded recipient per explicit spec — this is a
 * fixed business inbox, not an environment-configurable admin address
 * like sendAdminNewSignupNotification's ADMIN_NOTIFICATION_EMAIL.
 */
async function sendLeadMagnetNotification({
  resourceLabel,
  fullName,
  companyName,
  email,
  ...
  const html = `
    <div style="font-family:Arial,sans-serif">
      <h2>New ${escapeHtml(resourceLabel)} Lead</h2>
      <ul>
        ...
        <li><strong>Phone:</strong> ${escapeHtml(phone)}</li>
        <li><strong>Website:</strong> ${escapeHtml(websiteUrl)}</li>
        <li><strong>SEO Challenge:</strong> ${escapeHtml(challenge || '—')}</li>
        <li><strong>Date/Time:</strong> ${escapeHtml(new Date().toISOString())}</li>
        <li><strong>Source:</strong> ${escapeHtml(source)}</li>
        <li><strong>Landing Page:</strong> ${escapeHtml(landingPage)}</li>
      </ul>
    </div>
  `;
  ...
  return resend.emails.send({
    from: getFromAddress(),
    to: 'support@acendia.agency',
    subject: `New SEO Ebook Lead — ${companyName}`,
    html,
  });
}
...
module.exports = { sendWelcomeSetPasswordEmail, sendAdminNewSignupNotification, sendLeadMagnetNotification };
